...
# def q_learning(w ,h, p, r, gamma, alpha=0.01, epsilon_start=1.0, epsilon_end=0.01, epsilon_decay=0.999, threshold=0.01, max_episodes=1500, max_steps=100):
#     W, H = w ,h
#     Q = np.zeros((W, H, 4))
#     for x, y, v in L:
#         if v == 0:
...
#         valid_indices = [action_indices[a] for a in actions if is_valid_move(x + moves[a][0], y + moves[a][1], L, w, h)]
#         if not valid_indices:
#             return None
#         # Exclude the current action from consideration
#         valid_indices = [idx for idx in valid_indices if actions[idx] != current_action] 
#         if not valid_indices:  # No valid actions other than the current one
#             return None
...
# GPT:
def q_learning(w, h, L, p, r, gamma, alpha=0.01, epsilon_start=1.0, epsilon_end=0.01, epsilon_decay=0.999, threshold=0.01, max_episodes=5000, max_steps=100):
    Q = np.zeros((w, h, 4))
    for x, y, v in L:
        if v == 0:

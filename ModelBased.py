...
def run_experiments(W, H, L, p, r):
    ...
    # Calculating Frobenius norm between the q_values from the mbrl values and the MDP values
    q_values_max = np.max(grid_world.q_values, axis=2)  # Extract the max Q value for each state
    norm_diff = frobenius_norm(q_values_max, mdp.v_values)
    print(f"Frobenius norm between MBRL values and MDP values: {norm_diff:.3f}")

    # Calculating Frobenius norm between the q_values from Q-Learning and the v_values from MDP
    q_values_max = np.max(qlearning.q_values, axis=2)  # Extract the max Q value for each state
    norm_diff = frobenius_norm(q_values_max, mdp.v_values)
    print(f"Frobenius norm between MFRL values and MDP values: {norm_diff:.3f}")

    # Calculating Frobenius norm between the q_values from Q-Learning and the mbrl values
    q_values_max = np.max(qlearning.q_values, axis=2)  # Extract the max Q value for each state
    norm_diff = frobenius_norm(q_values_max, grid_world.q_values)
    print(f"Frobenius norm between MFRL values and MBRL values: {norm_diff:.3f}")
